  ...
  return (
    <>
      ...
      {showLiveNotification && (
        <div style={{ position: 'fixed', bottom: '6rem', left: '2rem', backgroundColor: 'white', padding: '0.75rem 1rem', borderRadius: '8px', boxShadow: '0 10px 25px rgba(0,0,0,0.15)', fontSize: '0.85rem', zIndex: 97 }}>
      ...
      {showExitPopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
          <div style={{ backgroundColor: 'white', padding: '2rem', borderRadius: '12px', textAlign: 'center', maxWidth: '400px', width: '100%', position: 'relative' }}>
            <button onClick={() => setShowExitPopup(false)} style={{ position: 'absolute', top: '1rem', right: '1rem', color: 'gray' }}>
              <X size={20} />
            </button>
            <h2 style={{ fontSize: '1.5rem', marginBottom: '0.5rem', color: 'var(--primary)' }}>⚡ Wait! Get Free Solar Quote</h2>
            <p style={{ color: 'gray', marginBottom: '1.5rem' }}>Save up to ₹75,000/year with solar.</p>
            <a href="https://wa.me/918882519245?text=Hello%20Balaji%20Entrepreneurs%2C%20I%20am%20interested%20in%20your%20solar%20solutions." style={{ display: 'block', backgroundColor: 'var(--secondary)', color: 'black', padding: '0.75rem 1rem', borderRadius: '8px', fontWeight: 600, width: '100%' }}>Get Quote Now</a>
          </div>
        </div>
      )}
    </>
